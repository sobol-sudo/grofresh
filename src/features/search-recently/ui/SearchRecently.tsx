...
export default function SearchRecently({ value, onValueChange }: SearchRecentlyProps) {
  ...
  /**
   * Edit mode only shows itself inside the recent-searches list — it is what puts the
   * delete crosses on the rows. Turning it on while the list is closed used to be a
   * tap that changed nothing on screen, so entering it opens the list as well.
   */
  const toggleEditMode = () => {
    const nextEdit = !edit;

    setEdit(nextEdit);
    if (nextEdit) setFocused(true);
  }
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            aria-label={edit ? 'Done editing recent searches' : 'Edit recent searches'}
            aria-pressed={edit}
            data-testid="toggle-edit-recent"
